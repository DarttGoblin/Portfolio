...
const projectLinks = ["https://darttgoblin.github.io/iBulk/iBulk.html"];
const latestProjectLinks = ["https://darttgoblin.github.io/LogSpectrum/Home.html"];
...
AnimateCaptions(ibulk_index, aca_nex_index, ls_index);
...
function ClickOnLink() {
    for (let i = 0; i < projectLinks.length; i++) {visitProject[i].onclick  = function() {
        if (projectLinks[i] == "notavailable") {alert('Website is not currectly available! check later.');}
        window.open(projectLinks[i], '_blank');}
    }
    for (let i = 0; i < latestProjectLinks.length; i++) {
        if (visitLatestProject[i] == "notavailable") {alert('Website is not currectly available! check later.');}
        visitLatestProject[i].onclick  = function() {window.open(latestProjectLinks[i], '_blank');}
    }
}
...
function SwitchDots() {
    if (dotsIndex == 3) {
        for (var i = 0; i < dots.length; i++) {StyleDots(dots[i], 0, "hidden");}
        dotsIndex = 0;
    }
    else {
...
function AnimateCaptions(ibulk_index, aca_nex_index, ls_index) {
